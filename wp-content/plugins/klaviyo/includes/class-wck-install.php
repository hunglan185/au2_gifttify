...
class WCK_Install {
  ...
  public static function remove_roles() {
    ...
    /**
     * Called from WCK_Api via the `disable` route. Cleanup Klaviyo plugin data (do not send a webhook since this
     * request comes from the app) and then deactivate the plugin.
     */
    public function deactivate_klaviyo()
    {
        $this->cleanup_klaviyo( $should_send_webhook = false );
     ...
     */
    public function cleanup_klaviyo_on_deactivation()
    {
        $this->cleanup_klaviyo();
    }

    /**
     * Handle cleanup of the plugin.
     * Delete options and remove WooCommerce webhooks.
     * Optionally send a webhook to remove the integration in Klaviyo to keep state aligned.
     *
     * @param bool $should_send_webhook Whether to send a webhook to Klaviyo to remove the integration.
     */
    private function cleanup_klaviyo( $should_send_webhook = true )
    {
        ...
        if ( is_plugin_active( 'woocommerce/woocommerce.php' ) ) {
            ...
            if ( $should_send_webhook ) {
                // Send remove webhook to Klaviyo
