...
if ( ! class_exists( 'S2W_IMPORT_SHOPIFY_TO_WOOCOMMERCE_ADMIN_Webhooks' ) ) {
	class S2W_IMPORT_SHOPIFY_TO_WOOCOMMERCE_ADMIN_Webhooks {
		...
		public function save_options() {
			global $s2w_settings;
			if ( ! current_user_can( apply_filters( 'vi_s2w_admin_sub_menu_capability', 'manage_options', 's2w-import-shopify-to-woocommerce-webhooks' ) ) ) {
				return;
			}
		...
		public function update_customer( $customer_data, $path ) {
			...
			if ( $existing_id ) {
				...
				$customer    = wp_parse_args( $customer, $default_address );
				...
				$wc_customer->set_shipping_phone( isset( $customer['phone'] ) ? $customer['phone'] : '' );
				...
				$wc_customer->set_billing_state( isset( $customer['province_code'] ) ? $customer['province_code'] : '' );
				$wc_customer->set_shipping_state( isset( $customer['province_code'] ) ? $customer['province_code'] : '' );
				$wc_customer->set_billing_country( isset( $customer['country_code'] ) ? $customer['country_code'] : '' );
				$wc_customer->set_shipping_country( isset( $customer['country_code'] ) ? $customer['country_code'] : '' );
				...
				if ( ! is_wp_error( $user_id ) ) {
					...
					if ( $user_id ) {
						...
						try {
							...
							self::log( $path, "Error importing customer #{$customer_data['id']}: {$e->getMessage()}, code: {$e->getCode()}" );
						}
					}
				} else {
					$user_id = false;
...
//				create order coupon lines
			...
			S2W_IMPORT_SHOPIFY_TO_WOOCOMMERCE::process_refunds( array(), $refunds, $order_id, $line_items_ids, $shipping_lines_id );
				...
				if ( count( $webhooks_orders_options ) ) {
					...
					if ( $order ) {
						...
						if ( in_array( 'line_items', $webhooks_orders_options ) ) {
							...
							$refunds      = $order_data['refunds'];
							S2W_IMPORT_SHOPIFY_TO_WOOCOMMERCE::process_refunds( $refund_items, $refunds, $existing_id, $line_items_ids, $shipping_lines_id );
							$order->save();
						}
						...
						do_action( 's2w_webhook_update_order_successfully', $existing_id, $order_data );
				...
				if ( count( $webhooks_products_options ) ) {
					...
					if ( $product ) {
						...
						if ( count( $options ) && count( $variants ) ) {
							...
							if ( in_array( 'tags', $webhooks_products_options ) ) {
					...
					if ( is_array( $options ) && count( $options ) ) {
						if ( count( $options ) == 1 && count( $options[0]['values'] ) == 1 ) {
							...
							if ( ! is_wp_error( $product_id ) ) {
								...
								do_action( 's2w_webhook_create_product_successfully', $product_id, $product_id );
							} else {
