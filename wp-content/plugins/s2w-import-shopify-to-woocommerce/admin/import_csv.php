...
class S2W_IMPORT_SHOPIFY_TO_WOOCOMMERCE_ADMIN_Import_Csv {
	...
	public function add_menu() {
		...
		add_submenu_page(
			...
			esc_html__( 'Import CSV', 's2w-import-shopify-to-woocommerce' ), apply_filters( 'vi_s2w_admin_sub_menu_capability', 'manage_woocommerce', $menu_slug ),
			$menu_slug, array(
				$this,
				'import_csv_callback'
			)
	...
	public function import_csv() {
		...
		if ( $pagenow === 'admin.php' && $page === 's2w-import-shopify-to-woocommerce-import-csv' ) {
			...
			if ( ! current_user_can( apply_filters( 'vi_s2w_admin_sub_menu_capability', 'manage_woocommerce', 's2w-import-shopify-to-woocommerce-import-csv' ) ) ) {
				return;
			}
...
//		if ( $product['handle'] && self::get_woo_product_id_by_slug( $product['handle'] ) ) {
//			return;
//		}
