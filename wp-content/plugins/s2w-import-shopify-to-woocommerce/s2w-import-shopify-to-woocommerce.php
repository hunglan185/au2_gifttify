...
define( 'VI_S2W_IMPORT_SHOPIFY_TO_WOOCOMMERCE_VERSION', '1.1.6' );
...
if ( ! class_exists( 'S2W_IMPORT_SHOPIFY_TO_WOOCOMMERCE' ) ) {
	class S2W_IMPORT_SHOPIFY_TO_WOOCOMMERCE {
		...
		public function delete_import_history() {
			global $pagenow;
			if ( ! current_user_can( apply_filters( 'vi_s2w_admin_menu_capability', 'manage_options', 's2w-import-shopify-to-woocommerce' ) ) ) {
				return;
			}
			if ( $pagenow === 'admin.php' && isset( $_GET['page'] ) && $_GET['page'] === 's2w-import-shopify-to-woocommerce' && isset( $_POST['s2w_delete_history'] ) ) {
				...
				if ( isset( $_POST['products'] ) && $_POST['products'] ) {
					VI_S2W_IMPORT_SHOPIFY_TO_WOOCOMMERCE_DATA::delete_option( 's2w_' . self::$settings->get_params( 'domain' ) . '_history' );
				...
				if ( isset( $_POST['coupons'] ) && $_POST['coupons'] ) {
					VI_S2W_IMPORT_SHOPIFY_TO_WOOCOMMERCE_DATA::delete_option( 's2w_' . self::$settings->get_params( 'domain' ) . '_history_coupons' );
					$files = glob( $path . '/coupons_*.txt' );
					VI_S2W_IMPORT_SHOPIFY_TO_WOOCOMMERCE_DATA::delete_files( $files );
				}
				if ( isset( $_POST['customers'] ) && $_POST['customers'] ) {
					VI_S2W_IMPORT_SHOPIFY_TO_WOOCOMMERCE_DATA::delete_option( 's2w_' . self::$settings->get_params( 'domain' ) . '_history_customers' );
					$files = glob( $path . '/customers_*.txt' );
					VI_S2W_IMPORT_SHOPIFY_TO_WOOCOMMERCE_DATA::delete_files( $files );
				}
				if ( isset( $_POST['orders'] ) && $_POST['orders'] ) {
					VI_S2W_IMPORT_SHOPIFY_TO_WOOCOMMERCE_DATA::delete_option( 's2w_' . self::$settings->get_params( 'domain' ) . '_history_orders' );
					$files = glob( $path . '/orders_*.txt' );
					VI_S2W_IMPORT_SHOPIFY_TO_WOOCOMMERCE_DATA::delete_files( $files );
				}
				if ( isset( $_POST['product_categories'] ) && $_POST['product_categories'] ) {
					VI_S2W_IMPORT_SHOPIFY_TO_WOOCOMMERCE_DATA::delete_option( 's2w_' . self::$settings->get_params( 'domain' ) . '_history_product_categories' );
				...
				if ( isset( $_POST['pages'] ) && $_POST['pages'] ) {
					VI_S2W_IMPORT_SHOPIFY_TO_WOOCOMMERCE_DATA::delete_option( 's2w_' . self::$settings->get_params( 'domain' ) . '_history_pages' );
				...
				if ( isset( $_POST['blogs'] ) && $_POST['blogs'] ) {
					VI_S2W_IMPORT_SHOPIFY_TO_WOOCOMMERCE_DATA::delete_option( 's2w_' . self::$settings->get_params( 'domain' ) . '_history_blogs' );
				...
				if ( isset( $_POST['store_settings'] ) && $_POST['store_settings'] ) {
					VI_S2W_IMPORT_SHOPIFY_TO_WOOCOMMERCE_DATA::delete_option( 's2w_' . self::$settings->get_params( 'domain' ) . '_history_store_settings' );
					VI_S2W_IMPORT_SHOPIFY_TO_WOOCOMMERCE_DATA::delete_files( $path . '/shop.txt' );
				}
				if ( isset( $_POST['taxes'] ) && $_POST['taxes'] ) {
					VI_S2W_IMPORT_SHOPIFY_TO_WOOCOMMERCE_DATA::delete_option( 's2w_' . self::$settings->get_params( 'domain' ) . '_history_taxes' );
					VI_S2W_IMPORT_SHOPIFY_TO_WOOCOMMERCE_DATA::delete_files( $path . '/shop.txt' );
				...
				if ( isset( $_POST['shipping_zones'] ) && $_POST['shipping_zones'] ) {
					VI_S2W_IMPORT_SHOPIFY_TO_WOOCOMMERCE_DATA::delete_option( 's2w_' . self::$settings->get_params( 'domain' ) . '_history_shipping_zones' );
		 ...
		 * @return array
		 */
		protected function initiate_data( $history_option, $domain, $access_token, $api_key, $api_secret, $type = 'order', $data_type = '' ) {
			$history = array(
				"total_{$type}s"               => 0,
		...
		public function save_and_check_key() {
			...
			foreach ( $args as $key => $arg ) {
				if ( isset( $_POST[ 's2w_' . $key ] ) ) {
					if ( is_array( $_POST[ 's2w_' . $key ] ) ) {
						$args[ $key ] = stripslashes_deep( $_POST[ 's2w_' . $key ] );
					} else if ( in_array( $key, array( 'fulfill_order_note' ) ) ) {
						$args[ $key ] = stripslashes( wp_kses_post( $_POST[ 's2w_' . $key ] ) );
					} else {
						$args[ $key ] = sanitize_text_field( stripslashes( $_POST[ 's2w_' . $key ] ) );
					}
				} else {
					if ( is_array( $arg ) ) {
						$args[ $key ] = array();
			...
			foreach ( $import_order_options as $import_order_option ) {
				if ( $args[ $import_order_option ] != self::$settings->get_params( $import_order_option ) ) {
					VI_S2W_IMPORT_SHOPIFY_TO_WOOCOMMERCE_DATA::delete_option( 's2w_' . $domain . '_history_orders' );
					$order_files = glob( VI_S2W_IMPORT_SHOPIFY_TO_WOOCOMMERCE_DATA::get_cache_path( $domain, $access_token, $api_key, $api_secret ) . '/orders_*.txt' );
			...
			foreach ( $product_import_options as $product_import_option ) {
				if ( $args[ $product_import_option ] != self::$settings->get_params( $product_import_option ) ) {
					VI_S2W_IMPORT_SHOPIFY_TO_WOOCOMMERCE_DATA::delete_option( 's2w_' . $domain . '_history' );
					$product_files = glob( VI_S2W_IMPORT_SHOPIFY_TO_WOOCOMMERCE_DATA::get_cache_path( $domain, $access_token, $api_key, $api_secret ) . '/product_*.txt' );
			...
			VI_S2W_IMPORT_SHOPIFY_TO_WOOCOMMERCE_DATA::update_option( 's2w_params', array_merge( self::$settings->get_params(), $args ) );
			$s2w_settings   = $args;
			...
			foreach ( $args as $key => $arg ) {
				if ( isset( $_POST[ $key ] ) ) {
					if ( is_array( $_POST[ $key ] ) ) {
						$args[ $key ] = stripslashes_deep( $_POST[ $key ] );
					} else if ( in_array( $key, array( 'fulfill_order_note' ) ) ) {
						$args[ $key ] = stripslashes( wp_kses_post( $_POST[ $key ] ) );
					} else {
						$args[ $key ] = sanitize_text_field( stripslashes( $_POST[ $key ] ) );
					}
				} else {
					if ( is_array( $arg ) ) {
						$args[ $key ] = array();
		...
		public function save_settings_order_options() {
			...
			$args = array(
				...
				'order_tag_to_status'      => isset( $_POST['order_tag_to_status'] ) ? array_map( 'stripslashes_deep', $_POST['order_tag_to_status'] ) : array(),
			);
		...
		public function save_settings_coupon_options() {
			...
			foreach ( $import_coupon_options as $import_coupon_option ) {
				if ( $args[ $import_coupon_option ] != self::$settings->get_params( $import_coupon_option ) ) {
					VI_S2W_IMPORT_SHOPIFY_TO_WOOCOMMERCE_DATA::delete_option( 's2w_' . $domain . '_history_coupons' );
					$coupon_files = glob( VI_S2W_IMPORT_SHOPIFY_TO_WOOCOMMERCE_DATA::get_cache_path( $domain, $access_token, $api_key, $api_secret ) . '/coupons_*.txt' );
		...
		public function save_settings_product_options() {
			...
			$args = array(
				...
				'use_external_image'          => isset( $_POST['use_external_image'] ) ? sanitize_text_field( $_POST['use_external_image'] ) : '',
				...
				'variable_sku'                => isset( $_POST['variable_sku'] ) ? str_replace( ' ', '', sanitize_text_field( $_POST['variable_sku'] ) ) : '',
				...
				'product_status_mapping'      => isset( $_POST['product_status_mapping'] ) ? stripslashes_deep( $_POST['product_status_mapping'] ) : array(),
				...
				'product_type_as'             => isset( $_POST['product_type_as'] ) ? sanitize_text_field( stripslashes( $_POST['product_type_as'] ) ) : '',
				'product_type_meta'           => isset( $_POST['product_type_meta'] ) ? str_replace( ' ', '', sanitize_text_field( $_POST['product_type_meta'] ) ) : '',
				'product_vendor_as'           => isset( $_POST['product_vendor_as'] ) ? sanitize_text_field( stripslashes( $_POST['product_vendor_as'] ) ) : '',
				'product_vendor_meta'         => isset( $_POST['product_vendor_meta'] ) ? str_replace( ' ', '', sanitize_text_field( $_POST['product_vendor_meta'] ) ) : '',
				'product_barcode_meta'        => isset( $_POST['product_barcode_meta'] ) ? str_replace( ' ', '', sanitize_text_field( $_POST['product_barcode_meta'] ) ) : '',
			);
		...
		public function sync() {
			...
			$download_description_images = self::$settings->get_params( 'download_description_images' );
			...
			if ( $domain && ( $access_token || ( $api_key && $api_secret ) ) ) {
				switch ( $step ) {
					case 'coupons':
						...
						if ( ! $import_history ) {
							$history_data = $this->initiate_data( $history_option, $domain, $access_token, $api_key, $api_secret, 'coupon', 'price_rules' );
					...
					case 'customers':
						if ( ! $import_history ) {
							$history_data = $this->initiate_data( $history_option, $domain, $access_token, $api_key, $api_secret, 'customer' );
							if ( $history_data['status'] == 'success' ) {
								$import_history = $history_data['data'];
								wp_send_json( array_merge( $import_history, array(
										'status' => 'retry'
											...
											if ( ! is_wp_error( $user_id ) ) {
												...
												if ( $user_id ) {
													...
													try {
														...
														VI_S2W_IMPORT_SHOPIFY_TO_WOOCOMMERCE_DATA::log( $log_file, 'Error: ' . $e->getMessage() . ', code: ' . $e->getCode() );
													}
												}
											}
										}
					...
					case 'orders':
						...
						if ( ! $import_history ) {
							$history_data = $this->initiate_data( $history_option, $domain, $access_token, $api_key, $api_secret );
							if ( $history_data['status'] == 'success' ) {
								$import_history = $history_data['data'];
								wp_send_json( array_merge( $import_history, array(
										'status' => 'retry'
...
//				create order coupon lines
											...
											update_post_meta( $order_id, '_s2w_shopify_order_number', apply_filters( 's2w_shopify_order_number', $order_data['order_number'], $order_data, $order_id ) );
							...
							if ( is_array( $products ) && count( $products ) ) {
								...
								for ( $key = $current + 1; $key < $max; $key ++ ) {
									...
									if ( is_array( $product_data ) && count( $product_data ) ) {
										...
										if ( $existing_id ) {
											self::import_product_vendor( $existing_id, $vendor );
											self::import_product_type( $existing_id, $product_type );
											$log['woo_id']      = $existing_id;
															...
															if ( ! is_wp_error( $product_id ) ) {
																...
																if ( is_array( $variants ) && count( $variants ) ) {
																	foreach ( $variants as $variant ) {
																		...
																		if ( $global_attributes ) {
																			$attributes = self::create_variation_global_attribute( $options, $variant );
																		} else {
																			$attributes = self::create_variation_custom_attribute( $options, $variant );
																		}
															...
															if ( ! is_wp_error( $product_id ) ) {
																...
																if ( is_array( $variants ) && count( $variants ) ) {
																	foreach ( $variants as $variant ) {
																		...
																		$barcode = apply_filters( 's2w_import_product_barcode', isset( $variant['barcode'] ) ? $variant['barcode'] : '', $variant );
																		self::import_product_barcode( $variation_obj->get_id(), $barcode );
																	}
																}
																do_action( 's2w_bulk_import_item_successfully', $product_id, $step, $product_data );
															}
												...
												if ( $product_id && is_array( $options ) && count( $options ) ) {
													...
													if ( is_array( $variants ) && count( $variants ) ) {
														if ( $download_images ) {
															...
															foreach ( $variants as $variant ) {
																vi_s2w_set_time_limit();
																if ( VI_S2W_IMPORT_SHOPIFY_TO_WOOCOMMERCE_DATA::sku_exists( $variant['sku'] ) ) {
																	$variation_id = wc_get_product_id_by_sku( $variant['sku'] );
																	if ( $variant['id'] == get_post_meta( $variation_id, '_shopify_variation_id', true ) ) {
																		continue;
																	}
		...
		public function page_callback_logs() {
            ...
            <div class="wrap">
				...
				$this->print_log_html( $logs );
				$logs = glob( VI_S2W_IMPORT_SHOPIFY_TO_WOOCOMMERCE_CACHE . '*/debug.txt' );
				$this->print_log_html( $logs );
				?>
            </div>
			<?php
                        ...
                        <form class="vi-ui form" method="post">
                                    ...
                                    <div class="<?php echo esc_attr( self::set( 'import-products-options-content' ) ) ?>">
                                        <div class="<?php echo esc_attr( self::set( 'import-products-options-heading' ) ) ?>">
                                            <div class="<?php echo esc_attr( self::set( 'save-products-options-container' ) ) ?>">
                                                <span class="vi-ui labeled icon primary button tiny <?php echo esc_attr( self::set( 'save-products-options' ) ) ?>"><i
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    <div>
														...
														$product_status_mapping = self::$settings->get_params( 'product_status_mapping' );
														if ( ! is_array( $product_status_mapping ) || ! count( $product_status_mapping ) ) {
															$product_status_mapping = self::$settings->get_default( 'product_status_mapping' );
														}
														?>
                                                        <table class="vi-ui table">
                                                            ...
                                                            <tbody>
															<?php
															foreach ( $product_status_mapping as $from => $to ) {
																?>
                                                                <tr>
                                    ...
                                    <div class="<?php echo esc_attr( self::set( 'import-orders-options-content' ) ) ?>">
                                        <div class="<?php echo esc_attr( self::set( 'import-orders-options-heading' ) ) ?>">
                                            <div class="<?php echo esc_attr( self::set( 'save-orders-options-container' ) ) ?>">
                                                <span class="vi-ui labeled icon primary button tiny <?php echo esc_attr( self::set( 'save-orders-options' ) ) ?>"><i
                                                            class="icon save"></i><?php esc_html_e( 'Save', 's2w-import-shopify-to-woocommerce' ) ?></span>
                                            </div>
                                                ...
                                                <td>
                                                    ...
                                                    <select name="<?php echo esc_attr( self::set( 'order_financial_status', true ) ) ?>"
														...
														foreach ( $financial_statuses as $financial_status_k => $financial_status_v ) {
															?>
                                                            <option value="<?php echo esc_attr( $financial_status_k ); ?>" <?php selected( $financial_status_k, self::$settings->get_params( 'order_financial_status' ) ) ?>><?php echo $financial_status_v ?></option>
															<?php
														}
														?>
                                                    </select>
                                                ...
                                                <td>
                                                    ...
                                                    <select name="<?php echo esc_attr( self::set( 'order_fulfillment_status', true ) ) ?>"
														...
														foreach ( $fulfillment_statuses as $fulfillment_status_k => $fulfillment_status_v ) {
															?>
                                                            <option value="<?php echo esc_attr( $fulfillment_status_k ); ?>" <?php selected( $fulfillment_status_k, self::$settings->get_params( 'order_fulfillment_status' ) ) ?>><?php echo $fulfillment_status_v ?></option>
															<?php
														}
														?>
                                                    </select>
                                    ...
                                    <div class="<?php echo esc_attr( self::set( 'import-coupons-options-content' ) ) ?>">
                                        <div class="<?php echo esc_attr( self::set( 'import-coupons-options-heading' ) ) ?>">
                                            <div class="<?php echo esc_attr( self::set( 'save-coupons-options-container' ) ) ?>">
                                                <span class="vi-ui labeled icon primary button tiny <?php echo esc_attr( self::set( 'save-coupons-options' ) ) ?>"><i
                                                            class="icon save"></i><?php esc_html_e( 'Save', 's2w-import-shopify-to-woocommerce' ) ?></span>
                                            </div>
                            ...
                            <p>
                                <span class="vi-ui labeled icon primary button <?php echo esc_attr( self::set( 'save' ) ) ?>"><i
                                            class="icon save"></i><?php esc_html_e( 'Save', 's2w-import-shopify-to-woocommerce' ) ?></span>
                                <button class="vi-ui labeled icon button" type="submit" name="s2w_check_key"><i
                    ...
                    <div class="vi-ui segment">
                        ...
                        <table class="vi-ui celled table center aligned">
							...
							if ( is_array( $elements ) && count( $elements ) ) {
								foreach ( $elements as $key => $value ) {
									if ( $key == 'products' ) {
										$history           = VI_S2W_IMPORT_SHOPIFY_TO_WOOCOMMERCE_DATA::get_option( 's2w_' . self::$settings->get_params( 'domain' ) . '_history', array(
											'total_products'         => 0,
											'total_pages'            => 0,
											'current_import_id'      => '',
											'current_import_product' => - 1,
										...
										$history_element = VI_S2W_IMPORT_SHOPIFY_TO_WOOCOMMERCE_DATA::get_option( 's2w_' . self::$settings->get_params( 'domain' ) . '_history_' . $key );
										$check           = 1;
...
//								$create_refunds[] = $create_refund;
		...
		public static function create_variation_custom_attribute( $options, $variant ) {
			$attributes = array();
			foreach ( $options as $option_k => $option_v ) {
				$j = $option_k + 1;
		...
		public static function create_variation_global_attribute( $options, $variant ) {
			$attributes = array();
			foreach ( $options as $option_k => $option_v ) {
				$j = $option_k + 1;
				if ( isset( $variant[ 'option' . $j ] ) && $variant[ 'option' . $j ] ) {
