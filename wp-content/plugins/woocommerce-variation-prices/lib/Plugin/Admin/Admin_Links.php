...
namespace Barn2\WVP_Lib\Plugin\Admin;

use Barn2\WVP_Lib\Plugin\Plugin;
use Barn2\WVP_Lib\Registerable;
use Barn2\WVP_Lib\Service;
...
class Admin_Links implements Registerable, Service {
	...
	public function add_settings_link( $links ) {
		...
		array_unshift( $links, sprintf( '<a href="%1$s">%2$s</a>', esc_url( $settings_url ), __( 'Settings', 'woocommerce-variation-prices' ) ) );
	...
	public function add_documentation_link( $links, $file ) {
		...
		$row_meta = [
			'docs' => sprintf(
				...
				esc_attr( sprintf( __( 'View %s documentation', 'woocommerce-variation-prices' ), $this->plugin->get_name() ) ),
				esc_html__( 'Docs', 'woocommerce-variation-prices' )
			)
		];
