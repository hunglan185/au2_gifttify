...
namespace Barn2\WVP_Lib\Plugin\License;

use Barn2\WVP_Lib\Registerable;
use Barn2\WVP_Lib\Util;
use DateTime;
...
class Plugin_License implements Registerable, License {
	...
	public function get_status_help_text() {
		...
		switch ( $this->get_status() ) {
			case 'active':
				$message = __( 'Your license key is active.', 'woocommerce-variation-prices' );
				break;
			case 'inactive':
				if ( ! $message ) {
			...
			default:
				if ( ! $this->get_license_key() ) {
					$message = __( 'Please enter your license key.', 'woocommerce-variation-prices' );
				} elseif ( ! $message ) {
					$message = __( 'Your license key is invalid.', 'woocommerce-variation-prices' );
				}
				break;
		}
		...
		switch ( $this->get_error_code() ) {
			case 'missing':
				...
				/* translators: 1: account page link open, 2: account page link close */
					__( 'Invalid license key - please check your order confirmation email or %1$sAccount%2$s.', 'woocommerce-variation-prices' ),
					Util::format_store_link_open( 'account' ),
					'</a>'
				);
				break;
			case 'missing_url':
				$message = __( 'No URL was supplied for activation, please contact support.', 'woocommerce-variation-prices' );
				break;
			case 'key_mismatch':
				$message = __( 'License key mismatch, please contact support.', 'woocommerce-variation-prices' );
				break;
			case 'license_not_activable':
				$message = __( 'This license is for a bundled product and cannot be activated.', 'woocommerce-variation-prices' );
				break;
			case 'item_name_mismatch':
			case 'invalid_item_id':
				$message = __( 'Your license key is not valid for this plugin.', 'woocommerce-variation-prices' );
				break;
			case 'no_activations_left':
				...
				if ( isset( $license_info['max_sites'] ) ) {
					...
					$limit = sprintf( _n( '%s site active', '%s sites active', absint( $license_info['max_sites'] ), 'woocommerce-variation-prices' ), $license_info['max_sites'] );
				}

				/* translators: %s: The sites active description, e.g. '2 sites active' */
				$message        = sprintf( __( 'Your license key has reached its activation limit (%s).', 'woocommerce-variation-prices' ), $limit );
				$read_more_link = Util::format_store_link( 'kb/license-key-problems', __( 'Read more', 'woocommerce-variation-prices' ) );

				/* translators: support for RTL. 1: license limit error message, 2: a read more link */
				$message = sprintf( __( '%1$s %2$s', 'woocommerce-variation-prices' ), $message, $read_more_link );
				break;
			case 'inactive':
			case 'site_inactive':
				$message = $this->get_license_inactive_message();
			...
			case 'expired':
				...
				if ( ! empty( $license_info['expires'] ) && is_numeric( substr( $license_info['expires'], 0, 4 ) ) ) {
					if ( $expiry_datetime = DateTime::createFromFormat( 'Y-m-d H:i:s', $license_info['expires'] ) ) {
						...
						$message = sprintf( __( 'Your license key expired on %s.', 'woocommerce-variation-prices' ), $expiry_datetime->format( get_option( 'date_format' ) ) );
					}
				}

				$renewal_link = Util::format_link( $this->get_renewal_url(), __( 'Renew now for 20% discount.', 'woocommerce-variation-prices' ), true );

				/* translators: support for RTL. 1: expired license error message, 2: a renewal link */
				$message = sprintf( __( '%1$s %2$s', 'woocommerce-variation-prices' ), $message, $renewal_link );
				break;
			case 'disabled':
				$message = $this->get_license_disabled_message();
				break;
	...
	private function get_license_inactive_message() {
		return __( 'Your license key is not active. Please reactivate or save the settings.', 'woocommerce-variation-prices' );
	}

	private function get_license_expired_message() {
		return __( 'Your license key has expired.', 'woocommerce-variation-prices' );
	}

	private function get_license_disabled_message() {
		return sprintf(
			__( 'Your license key has been disabled. Please %1$spurchase a new license key%2$s to continue using the plugin.', 'woocommerce-variation-prices' ),
